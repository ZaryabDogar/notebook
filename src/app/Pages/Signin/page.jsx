...
function Signin() {
	...
	const handelsubmit = async (e) => {
		...
		const response = await fetch(
			`https://note-be-two.vercel.app/api/auth/login`,
			{
				method: 'POST',
				headers: {
		...
		if (json.success) {
			...
			if (Array.isArray(json.error)) {
				...
				showalert(
					`Unexpected error occurred: ${JSON.stringify(json.error)}`,
					'red',
					'flex'
				);
			} else {
								...
								<button
									...
									className={`${
										...
										credentials.email.length < 5
											? 'focus:ring-2 focus:ring-offset-2 focus:ring-indigo-400 text-sm font-semibold leading-none text-gray-400 focus:outline-none bg-indigo-400 border rounded hover:bg-indigo-500 py-4 w-full'
